...
for T_L in T_L_VALUES:
    ...
    ax_tr.errorbar(gs, tr, yerr=e_tr, fmt="o-", capsize=3, label=rf"$T=L_{{target}}={T_L}$")
    ax_te.errorbar(gs, te, yerr=e_te, fmt="s-", capsize=3, label=rf"$T=L_{{target}}={T_L}$")

for ax, title in ((ax_tr, r'$d_F(M_{out}, M_{target})$'),
                   (ax_te, r'$d_F(M_{out}, M_{target})$')):
    ax.set_xlabel(r"$g$", fontsize=20)
    ax.set_ylabel(title, fontsize=20)
    # ax.set_yscale("log")  # linear is easier to read for cross-depth comparison
    ax.grid(alpha=0.3)
    ax.set_title(f"Final {'training' if ax is ax_tr else 'testing'} losses", fontsize=20)
    ax.legend()
...
fig.savefig(out, dpi=150)
out = os.path.join(FIG_DIR, f"bath_sweep_N{N}_final_vs_g_all_TL.pdf")
fig.savefig(out, dpi=150)
print(f"saved {out}")
