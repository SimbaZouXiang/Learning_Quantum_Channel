#!/bin/bash
# DJ4: top-up TL=5 multi-seed to 20 seeds total. Adds offsets 113..119 (7 seeds
# × 10 g = 70 tasks) at 4 cores/worker × 48 per batch → 2 batches × ~25 min.
set -uo pipefail
...
LOGDIR=slurm_outputs/multiseed_DJ4_TL5_$(date +%Y%m%d-%H%M%S)
mkdir -p "$LOGDIR"
echo "[DJ4 TL=5] logs $LOGDIR" | tee "$LOGDIR/driver.log"
...
TOTAL=${#TASKS[@]}
echo "[DJ4 TL=5] total tasks: $TOTAL  batch cap: $BATCH_CAP  cores/worker: $CORES_PER_TASK" \
...
while [ $i -lt $TOTAL ]; do
    ...
    echo "[DJ4 TL=5] -- batch $batch_num starting at task $i --" | tee -a "$LOGDIR/driver.log"
    PIDS=()
    slot=0
    ...
    echo "[DJ4 TL=5] batch $batch_num: ${#PIDS[@]} workers, waiting..." | tee -a "$LOGDIR/driver.log"
    for pid in "${PIDS[@]}"; do wait "$pid"; done
    echo "[DJ4 TL=5] batch $batch_num done" | tee -a "$LOGDIR/driver.log"
done
...
echo "[DJ4 TL=5] ✓ full save: $ok  △ post-train only: $partial  ✗ no save: $bad" \
    | tee -a "$LOGDIR/driver.log"
