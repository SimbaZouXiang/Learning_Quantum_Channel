#!/bin/bash
#SBATCH --job-name=bath-TL5-pk
#SBATCH --output=slurm_outputs/slurm-bath-TL5-pk-%j.out
#SBATCH --time=20:00:00
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=192
# T_L=5, max_bd=64, use_compressed=True, packed: 6 workers/node × 32 cores each.
# Per-task wallclock estimate: ~75 min on 32-core chiplet. Per-batch ~75 min,
# 2 batches = ~150 min total; 10 h cap gives ~4× margin.
set -uo pipefail
...
N_PARALLEL=6
N_G=11
LOGDIR=slurm_outputs/packed_TL5_${SLURM_JOB_ID}
mkdir -p "$LOGDIR"
echo "[TL5 packed] job ${SLURM_JOB_ID}, logs in $LOGDIR" | tee "$LOGDIR/driver.log"

for BATCH_START in 0 6; do
    echo "[TL5 packed] batch starting at g_idx=$BATCH_START" | tee -a "$LOGDIR/driver.log"
    PIDS=()
    for offset in $(seq 0 $((N_PARALLEL-1))); do
        ...
        (
            ...
            echo "[TL5 packed] worker g_idx=$IDX exit=$?" | tee -a "$LOGDIR/driver.log"
        ) &
        PIDS+=($!)
        echo "[TL5 packed]   launched g_idx=$IDX on cores $START-$END" | tee -a "$LOGDIR/driver.log"
    done
    for pid in "${PIDS[@]}"; do wait "$pid"; done
    echo "[TL5 packed] batch done" | tee -a "$LOGDIR/driver.log"
done
