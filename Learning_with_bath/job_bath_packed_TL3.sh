#!/bin/bash
#SBATCH --job-name=bath-TL3-pk
#SBATCH --output=slurm_outputs/slurm-bath-TL3-pk-%j.out
#SBATCH --time=08:00:00
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=192
# T_L=3, max_bd=64, use_compressed=True, packed: 6 workers/node × 32 cores each.
# 11 g values cover in 2 sequential batches of 6 (last batch has 5 idle slots).
# Per-task wallclock estimate: ~30 min on 32-core chiplet. Per-batch ~30 min,
# 2 batches = ~60 min total; 4 h cap gives ~4× margin.
set -uo pipefail
...
N_PARALLEL=6
N_G=11
LOGDIR=slurm_outputs/packed_TL3_${SLURM_JOB_ID}
mkdir -p "$LOGDIR"
echo "[TL3 packed] job ${SLURM_JOB_ID}, logs in $LOGDIR" | tee "$LOGDIR/driver.log"

# Run two sequential batches. Batch 0 handles g_idx 0..5, batch 1 handles 6..10.
for BATCH_START in 0 6; do
    echo "[TL3 packed] batch starting at g_idx=$BATCH_START" | tee -a "$LOGDIR/driver.log"
    PIDS=()
    for offset in $(seq 0 $((N_PARALLEL-1))); do
        ...
        (
            ...
            echo "[TL3 packed] worker g_idx=$IDX exit=$?" | tee -a "$LOGDIR/driver.log"
        ) &
        PIDS+=($!)
        echo "[TL3 packed]   launched g_idx=$IDX on cores $START-$END" | tee -a "$LOGDIR/driver.log"
    done
    for pid in "${PIDS[@]}"; do wait "$pid"; done
    echo "[TL3 packed] batch done" | tee -a "$LOGDIR/driver.log"
done
