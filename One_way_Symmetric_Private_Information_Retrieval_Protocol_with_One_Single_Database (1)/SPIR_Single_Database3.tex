...
Now, we will take into account generalized measurement. Suppose Alice has the ability to conduct a generalized measurement, that we will show she still gains no partial information from a non-deterministic result.

Without loss of generosity, suppose Bob sends Alice a state, and announce that the state is either $\ket{0}$ or $\ket{-}$, then, Alice can define a generalized POVM elements $Pp, Pq, Pr$, where $Pp+Pq+Pr=I_3$, such that 
...
Regardless of the noise level, the number of unambiguously determined states per block follows a Gaussian distribution, where the probability of a block with $x$ unambiguously determined states decrease exponentially with $x$ when $x$ is large. This gives the asymptotic security against Alice cheating of our protocol.
...
One curious question that can be asked is that, in our main protocol, Bob sends the qubits to Alice and Alice perform USD one-by-one. One may wonder if instead of doing it successively, if Alice do a collective USD on multiple received qubits, will it provides any advantages over? The answer is no. This is proven in many literature, for example Ref.~\cite{USD_one_by_one}.
