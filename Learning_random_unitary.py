...
def run_single_task(args):
    ...
    # We want each worker to use its allocated share of cores natively in PyTorch
    # during the sequential training loop, rather than locking to 1 core.
    ...
    # TDME_Trott moved from the (old) flat /scratch/simba layout into
    # Learning_Lindbladian/ (now a shim over the qcl package) — make it
    # importable regardless of CWD.
    script_dir = os.path.dirname(os.path.abspath(__file__))
    lindblad_dir = os.path.join(script_dir, "Learning_Lindbladian")
    if lindblad_dir not in sys.path:
        sys.path.insert(0, lindblad_dir)
    ...
    (model, learning_loss, target_param, p_depolar,
     ...
     model_p_dephaseX, model_p_dephaseY, model_p_dephaseZ
        ...
        num_threads=1,  # Use single Python thread, rely on PyTorch native parallelism
    ...
    prefix = f"Learning_random_unitary/random_training_set/Depolarizing_N{N}_T{T}_L{L}_p_{depolarizing_name:03d}"
...
def main():
    ...
    N = 8
    T = 3
    depolarizing_names = [1, 2, 3, 4, 5, 6]
    target_layers      = [3, 4, 5, 6, 7, 8, 15, 25]
    epochs       = 200
    lr           = 0.05
    normalized   = False
    truncation   = True
    ...
    num_tasks   = len(tasks)
    num_workers = args.workers if args.workers else min(num_tasks, 16)
    num_workers = min(num_workers, num_tasks, TOTAL_CORES)

    # Give each worker a fair share of cores (for BLAS/OMP, not Python threads)
    # Cap thread count to prevent OOM when computing tensor networks for large L
    threads_per_worker = min(100, max(1, TOTAL_CORES // num_workers))
