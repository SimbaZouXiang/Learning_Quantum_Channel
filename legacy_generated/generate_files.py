...
for p in [6, 8, 10]:
    for l_name, l_val in [("345", "3, 4, 5"), ("67", "6, 7")]:
        py_file = f"LRU_p{p}_L{l_name}.py"
        sh_file = f"job_p{p}_L{l_name}.sh"
        job_name = f"LRU_p{p}_L{l_name}"
        
        with open(py_file, "w") as f:
            f.write(py_template.replace("{DEP}", str(p)).replace("{LAYERS}", l_val))
            
        with open(sh_file, "w") as f:
            f.write(sh_template.replace("{JOB_NAME}", job_name).replace("{PY_FILE}", py_file))
