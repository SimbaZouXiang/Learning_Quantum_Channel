"""Train w2-only (full weight-2 basis, 252 inputs, no weight-1) for the standard
grid, saving full student parameters to ./results/. Same target seeds as the
prior w1 / combined runs so the resulting trained channel can be directly
compared.

Output filename prefix: N{N}_T{T}_L{L}_p{p}_w2full_*
"""
...
def _make_target_param(L_target, p):
    """Same seed as the prior runs so the teacher is identical."""
    import torch
...
def _worker(L_target, depol):
    ...
    seed = 42 + L_target * 100 + int(round(depol * 1000)) * 10 + 3  # distinct from w1 (+1) and combined (+4)
...
def main():
    ...
    print(f"  Tasks: {n_tasks}  Workers: {n_workers}  (252 weight-2-full inputs/training)", flush=True)
