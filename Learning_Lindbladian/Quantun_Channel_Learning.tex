...
\PassOptionsToPackage{draft}{graphicx}% draft: render placeholder boxes for missing figure files
\usepackage{tikz}
...
\begin{enumerate}
    \item A channel-learning framework based on the Frobenius distance between Pauli transfer matrices, which is upper-bounded by the diamond distance~\cite{nielsen2010quantum} and is directly computable from tensor-network contractions without ancillas.
    ...
    \item A closed-form prediction of the target depth $L_{\max}(p,T)$ at which the learning loss peaks, giving the scaling $L_{\max}\sim p^{-1/2}$ at small noise, in contrast with the noise-limited depth $\sim 1/p$ governing global-purity decay~\cite{aharonov2023polynomial,deshpande2022tight}.
...
\begin{equation}
    ...
    \label{eq:diamond}
\end{equation}
where $\mathrm{Id}_k$ denotes the identity channel on a $k$-dimensional ancilla and $\mathcal{D}(\cdot)$ the set of density operators. By Stinespring's theorem, the supremum is attained at $k=d$~\cite{watrous2018theory}. The diamond distance upper-bounds the operationally relevant quantity $\sup_\rho\,\tfrac{1}{2}\norm{\E_1(\rho)-\E_2(\rho)}_1$, i.e.\ the worst-case distinguishability of the two channels even in the presence of arbitrary entangled inputs.
...
\end{equation}
The upper bound is saturated only when all $d^2$ singular values of $J(\E_1-\E_2)$ are comparable; for channels close to the identity (as in our compression setting) the effective rank is far smaller than $d^2$, so the tighter inequality $\norm{\E_1-\E_2}_\diamond \le \sqrt{r}\,\norm{J(\E_1-\E_2)}_F$ with $r\ll d^2$~\cite{watrous2018theory} applies, and the Frobenius loss remains a physically meaningful surrogate for channel distinguishability. \todoflag{Eq.~\eqref{eq:diamond_vs_PTM_Frob} substantiates item~(3) of the bullet list justifying the Frobenius loss.}
...
\section{Parametrized Quantum Circuit Topology}
...
In general, the Ansatz PQC can have $T$ layers, for any $T\in \mathbb{Z}^+$.
...
Then, in each training epoch, we pass $\{M_{in}\}_k$ to our Ansatz PQC and obtain the output MPS in this epoch $\{M_{out}\}_k = \{\E_{model}(M_{in})\}_k$. Then, we compute the mean Frobenius distance between the set $\{M_{realout}\}_k$ and $\{M_{out}\}_k$.
...
Hence, the Frobenius norm of the MPS $M$ is suppressed by a factor of $(1-4p/3)^w$ per layer of depolarizing operator with strength $p$. Hence, the higher the initial Pauli weight of the MPS $M$, the smaller the $\norm{M_{out}}_F$. Thus, to generate a useful training set, we use the first non-trivial set of the Pauli strings, which is the set of all weight-1 Pauli strings.

\subsubsection{Weight-1 sufficiency in the intermediate-noise regime}
...
As an immediate corollary, one layer of the PQC family is uniquely determined by its action on weight-1 Paulis.
...
\begin{figure}
    ...
    \caption{\raggedright The average Frobenius distance between passing through a weight-1 Pauli string through a $T_{PQC}$ layer PQC with and without truncation, with depolarizing noise $p=0.04$.}
...
\paragraph*{Linear LOE growth for unitary dynamics.}
For unitary circuits, Prosen and Pi\v{z}orn~\cite{prosen2007operator} and Dubail~\cite{dubail2017entanglement} established that the Local Operator Entanglement grows \emph{linearly} in the circuit depth, saturating only at layer to be $\mathcal{O}(N)$. In the brickwork setting, one can write
\begin{equation}
...
To make the trotterization simulation more accurate at fixed depth, we use the second-order Strang-symmetrized splitting whenever a high-fidelity reference channel is required. Concretely, one second-order Trotter layer applies the dissipator $M$ for a half step, then sweeps the Hamiltonian unitary brickwall as odd--even--odd around it, and closes with the dissipator $M$ for a half step. This corresponds to the per-layer pattern
$
\mathrm{Diss}(\Delta t/2)\circ U^o(\Delta t/2)\circ U^e(\Delta t)\circ U^o(\Delta t/2)\circ \mathrm{Diss}(\Delta t/2)
$
visible in Figure~\ref{fig:Trotterization_Circuit}, with per-layer error $\mathcal{O}(\Delta t^3)$. The first-order splitting $\mathrm{Diss}(\Delta t)\circ U^o(\Delta t)\circ U^e(\Delta t)$, in contrast, contains only one dissipator and one odd--even brickwall per layer, with per-layer error $\mathcal{O}(\Delta t^2)$.

\paragraph{Notation.} Throughout this section the following channel-level notation is used:
\begin{itemize}
    \item $\E_{model}$: the $T$-layer PQC ansatz that we train. Each PQC layer consists of a parameterized odd--even brickwall of trainable two-qubit $SU(4)$ gates followed by per-site noise channels (depolarizing and dephasing); a worked example with $T=2$ is the Ansatz of Figure~\ref{fig:Ansatz_circuit}. The model parameters are written into $\E_{model}$ implicitly. We use $T=3$ throughout the numerical experiments of this section.
    \item $\E_{Trott,r}^{(p)}$: the channel obtained by compiling the time-dependent Lindbladian of Eq.~\ref{eq:TDME_Lindbladian} into $r$ Trotter layers using the $p$-th order Suzuki--Trotter splitting at total evolution time $t$ and per-step time $\Delta t = t/r$. The superscript $(p)\in\{1,2\}$ records the splitting order and disambiguates the per-layer gate pattern; in particular $\E_{Trott,r}^{(1)}$ has the same per-layer ``shape'' as a single PQC layer (one dissipator sublayer plus one odd--even brickwall, no symmetrization), while $\E_{Trott,r}^{(2)}$ has the symmetric Strang pattern shown in Fig.~\ref{fig:Trotterization_Circuit}.
    \item $\E_{Trott,30}^{(2)}$: the deep, near-exact reference channel that we treat as the ``ground truth'' the model is trying to compress. We use $r=30$ and $p=2$ everywhere a target is needed.
    \item $\E_{Trott,3}^{(1)}$ and $\E_{Trott,3}^{(2)}$: the two short Trotter benchmarks at the same depth as the model. They are not learned -- they are the uncalibrated $r=3$ Trotter compilations at the indicated splitting order, used to put the PQC's testing loss into perspective at fixed circuit depth.
    \item $M_{in,i}$, $M_{test,i}$: training inputs (a complete weight-$1$ basis of size $3N$) and testing inputs (300 random Pauli strings with weight $w\sim\mathrm{Unif}[1,N]$), respectively. We write $M_{out,i}=\E_{model}(M_{in,i})$, $M_{Trott,r,i}^{(p)}=\E_{Trott,r}^{(p)}(M_{in,i})$ for the corresponding outputs, with the test-set analogues obtained by replacing $M_{in,i}$ with $M_{test,i}$.
    \item $d_F(\cdot,\cdot)$: the Frobenius distance between two operator-valued matrix product states defined in Sec.~\ref{eq:TDME_Lindbladian}; this is the per-sample contribution to all training and testing losses below.
\end{itemize}
With this notation, the previously written $\E_{Trott,30}$ in earlier work corresponds to $\E_{Trott,30}^{(1)}$ when the underlying compilation is first-order and $\E_{Trott,30}^{(2)}$ otherwise; we make the order explicit from this point on whenever it matters for the comparison.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{pqc_vs_trotter.pdf}
...
As a final case study of our Quantum machine learning model using PQCs, we want to see if the compression can be done when quantum system is coupled to quantum baths. In the previous sections, we are mainly focused on whether we can compress a given quantum circuit with a smaller PQC, the target circuit can be either a larger PQC or the Trotterization simulation on some Lindbladian dynamics. In other words, the previous numerical parts of this paper focus on compressing the quantum circuit in the depth direction.

In this section, we ask a different question, that is, whether a quantum circuit can be compressed in the width-direction. The system we are interested in, is a small quantum system that are coupled to an external large quantum bath, and we want to understand if it is possible theoretically to use a much smaller bath to simulate the effect of the larger bath that couples to the quantum system.

So here, we study a 8 qubit problem, where there are 8 system qubits, and we have a large bath that is simulated with 16 qubit sites, and there are unitary interactions between the bath and the system qubits. We would like to learn this with a smaller bath with 8 qubits, and see if they give similar results.
