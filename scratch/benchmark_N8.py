...
def run_benchmark():
    ...
    for threads in threads_list:
        ...
        Learning_TDME_scheduler(N=N, MPO_layer=MPO_layer, model_to_learn_layer=model_to_learn_layer, mu=mu, gamma=gamma, epochs=0, use_scheduler=False, num_threads=threads)
        elapsed = time.time() - start_time
        times.append(elapsed)
        print(f"-> Time N={N} with {threads} thread(s): {elapsed:.2f} seconds\n")
